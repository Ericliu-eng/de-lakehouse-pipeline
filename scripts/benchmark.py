...
The benchmark replays Alpha Vantage payloads already saved under data/raw, so it
makes no API calls. It creates a dedicated PostgreSQL database (default
...
from de_lakehouse_pipeline import backfill, pipeline
...
def render(env, inputs, inc, qm, atomic, bf, retry, lat) -> str:
    symbols = ", ".join(inputs["symbols"])
    lines = [
        f"# Pipeline Benchmark — {env['date']}",
        "",
        "Generated by `python -m scripts.benchmark`. Saved Alpha Vantage payloads were replayed "
        "into a dedicated PostgreSQL database, so no API calls were made and the development "
        "database was not modified. Timings are single-machine local measurements, not "
        "production SLAs.",
        "",
        "| Environment | Value |",
        "| --- | --- |",
    ...
    lines += [
        ...
        "| Metric | Result |",
        "| --- | --- |",
        f"| Warehouse size | {inc['total_rows']:,} rows · {inc['symbols']} symbols · "
